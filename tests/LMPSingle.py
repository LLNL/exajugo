    ...
    for d in Day:
        ...
        if os.path.exists(file):
            ...
            if r not in all_L_values:
                all_L_values[r] = []
            all_L_values[r].extend(df['LMP'].tolist())
...
for r in average_L_over_time:
    ...
    plt.plot(range(1,25), avg_l_over_t.values, linestyle=linestyle_tuple[i], linewidth=3, marker='o', label=f'$R_b$ = {r}')
    i += 1
...
plt.xticks(np.arange(0,25,4),fontsize=20, fontname='Arial') 
