...
for sheet_name in ['Lactive', 'Tactive']:
    ...
    for i, (file_name_A, file_name_B) in enumerate(zip(file_names_A, file_names_B)):
        ...
        axs[row, col].scatter(x_A2, y_A2, alpha=0.4, marker='o', label=Setmeaning[set1], color='red')
        # axs[row, col].scatter(x_A2, y_A2, alpha=0.8, marker='x', label=Setmeaning[set1], color='blue')
        # axs[row, col].set_xticks(unique_x_values)
        # # Rotate the x-labels to prevent overlapping
        # axs[row, col].set_xticklabels(unique_x_values, rotation=45)
        axs[row, col].scatter(x_B2, y_B2, alpha=0.8, marker='x', label=Setmeaning[set2], color='blue')
        # axs[row, col].set_xticks(unique_x_values)
        # # Rotate the x-labels to prevent overlapping
        # axs[row, col].set_xticklabels(unique_x_values, rotation=45)
        
        # Set labels and title for the subplot
        axs[row, col].set_xlabel('line #' if sheet_name == 'Lactive' else 'transformer #', fontname='Arial')
        axs[row, col].set_ylabel('time (hour)', fontname='Arial')
        axs[row, col].set_title(f'R= {BRatio[i]}',fontname='Arial')
        axs[row, col].set_ylim(0,25)
        axs[row, col].set_yticks([0,4,8,12,16,20,24])
        # axs[row, col].legend()
