...
function callMPACOPF(RAWFILE::String, ROPFILE::String, INLFILE::String, CTfile::String, plantfile::String)
	...
	#  writepara(TMPDIR, G, N, plant_unsort,Day)
	# return 
		...
		MPACOPFSol =
			try
				...
				elseif MethodSol == "Preg"
					...
					SolveMPACOPF(o, vN, vL, vT, vSSh, vG, vK, vP, timestep, plant, MVAbase, MPACOPF.ipopt_optPrint())
				end
	...
	clock = @elapsed begin
		...
		pmslack, ppslack, qmslack, qpslack, lslack, tslack, Lloss, Gtot, eta, pline, ptran, Genbus, Llossbus, gencoststep, Laconst, Taconst = MPsolstate(MPACOPFSol)
		writesolutionAllBuses(TMPDIR, vN, timestep, plant, Genbus, Llossbus,LMPs, LMPsN, RatioBidding, Day)
		# return
		writesolutionBid(TMPDIR, MVAbase, vN, vSSh, vG, generators, plant, timestep, v_n, 
			theta_n, b_s, p_g, q_g, prodcost, basepen, plantrev, plantpower, LMPs, LMPsN, DBratio, RatioBidding, Day) 
		writesolutionPlant(TMPDIR, vN, timestep, plantpower, plant, LMPs, DBratio, RatioBidding, Day)
		# writesolutionSlack(TMPDIR, pmslack, ppslack, qmslack, qpslack, lslack, tslack, vN, vL, vT, timestep, RatioBidding, Day)
		writeloss(TMPDIR, Lloss, Gtot, eta, pline, ptran, gencoststep, Laconst, Taconst, vL, vT, timestep, RatioBidding, Day)
...
function MPsolstate(MPACOPFSol::MPACOPF.MPACOPFBaseState)
	...
	Genbus = MPACOPFSol.TLoss.Gentotbus
	Llossbus = MPACOPFSol.TLoss.Llossbus
	gencoststep = MPACOPFSol.TLoss.GenCostStep
	Lconst = MPACOPFSol.TLoss.Lactive
	Tconst = MPACOPFSol.TLoss.Tactive

	return v_n, theta_n, b_s, p_g, q_g, prodcost, basepen, plantrev, plantpower, LMPs, LMPsN,
		pmslack, ppslack, qmslack, qpslack, lslack, tslack, Lloss, Gtot, eta, pline, ptran, Genbus, Llossbus, gencoststep, Lconst, Tconst
end
