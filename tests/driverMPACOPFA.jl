...
clock = @elapsed begin
	...
	using InstanceReaderMP, MPACOPF, MPACOPFA, SolutionWriterMP
...
function callMPACOPF(RAWFILE::String, ROPFILE::String, INLFILE::String, CTfile::String, plantfile::String)
		...
		MPACOPFSol =
			try
				...
				elseif MethodSol == "Preg"
					...
					SolveMPACOPFA(o, vN, vL, vT, vSSh, vG, vK, vP, timestep, plant, MVAbase, MPACOPFA.ipopt_optPrint())
				end
	...
	clock = @elapsed begin
		...
		writesolutionAllBuses(TMPDIR, vN, timestep, plant, Genbus, Llossbus,LMPs, LMPsN, RatioBidding)
		# return
		...
		writeloss(TMPDIR, Lloss, Gtot, eta, pline, ptran, gencoststep, Laconst, Taconst, vL, vT, timestep, RatioBidding)
