...
for r in BRatio:
    for d in Day:
        ...
        if os.path.exists(file):
            ...
            # all_L_values[r][d].extend(df['plantpower'].tolist())  
        else:
...
# plt.plot(BRatio, overall_mean_vals, label='Average', color='red', linestyle='-', linewidth=5)
...
# plt.title('Average generation cost at different bidding ratio, single-bus setting', fontsize=25, fontname='Arial')
...
# plt.ylabel('plant power (MW)', fontsize=25, fontname='Arial')
# plt.title('Average bidding plant power at different bidding ratio, single-bus setting', fontsize=25, fontname='Arial')
...
plt.gca().xaxis.set_major_locator(plt.MultipleLocator(0.05))    # Uniform grid spacing of 1 for x-axis
