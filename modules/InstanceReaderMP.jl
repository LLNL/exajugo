...
export readinstance, GOfmt2params, DELTA, ParseInstance, GOfmt2paramsMP
...
function biddingload(plant::DataFrame, MVAbase::Float64)

		# scale up the plant & change unit to MW/MVA
		plant[!,:pplantub] .*= 10.0/MVAbase
		plant[!,:pplantlb] .*= 10.0/MVAbase
		plant[!,:plantdemand] .*= 10.0
...
## function to calculate total load demand and plant demand
...
function demandload(buses::DataFrame, loads::DataFrame, plant::DataFrame, RatioBidding::Float64, MVAbase::Float64) 
	...
	println("demandtot(MW), DBratio, num_plant ", demandtot*MVAbase," ", DBratio, " ", num_plant)
...
function plantloads(changetable::DataFrame, loads::DataFrame, plant::DataFrame, RatioBidding::Float64, MVAbase::Float64)

	# find # of plants to replace loads, record the actually bidding load ratio to total load
	numPlant, DBratio, demandtotal = mindemandload(changetable, plant, RatioBidding, MVAbase)
	# numPlant, DBratio, demandtotal = demandload(buses, loads, plant, RatioBidding, MVAbase)
	...
	return loadsPartial, plant_sorted, plant, DBratio, demandtotal
end
...
function GOfmt2paramsMP(MVAbase::Float64, buses::DataFrame, loads::DataFrame, fixedbusshunts::DataFrame,
	...
	# scale up the single plant
	plantbid = biddingload(plant, MVAbase)
	# multiple plants on different buses
	loadsPartial, plantRep, plant_unsorted, DBratio, demandtotal = plantloads(changetable, loads, plantbid, RatioBidding, MVAbase)
	...
	for tstep in timestep
		...
		# println("change table selected: ", change_tstep)
		...
		if size(loadsPartial,1) > 0
			for n = eachindex(plantRep[!,:plantid])
				...
				# test effect of reducing demand by 1MW
				# if loadsCT[!,:I][Lidx] == 7229
				# 	# println("before ", loadsCT[!,:PL][Lidx])
				# 	loadsCT[!,:PL][Lidx] = loadsCT[!,:PL][Lidx] - 1
				# 	# println("after", loadsCT[!,:PL][Lidx])
				# end
			end
		# else
		# 	# test effect of reducing demand by 1MW when there is no plant
		# 	loadsCT[!,:PL] = @. ifelse(loadsCT[!,:I] == 7229, loadsCT[!,:PL]-1, loadsCT[!,:PL])
		end
	...
	return vN, vL, vT, vSSh, vG, vK, vP, timestep, plantRep, plant_unsorted, DBratio
